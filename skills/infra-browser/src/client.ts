...
import { acquirePageLock, releaseAllLocks } from "./publish-lock";
...
/**
 * Set files on a file input element via the server (bypasses CDP 50MB transfer limit).
 * Use this for uploading large files that would fail with page.setInputFiles().
 */
export async function setFileServerSide(
  pageName: string,
  filePaths: string | string[],
  options: { selector?: string; frameUrl?: string; serverUrl?: string } = {}
): Promise<{ success: boolean; method?: string; error?: string }> {
  const { selector, frameUrl, serverUrl: url = "http://localhost:18273" } = options;
  const res = await fetch(`${url}/pages/${encodeURIComponent(pageName)}/set-file`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ filePaths, selector, frameUrl }),
  });
  return res.json() as Promise<{ success: boolean; method?: string; error?: string }>;
}

export async function connect(serverUrl = "http://localhost:18273"): Promise<DevBrowserClient> {
  ...
  async function getPage(name: string): Promise<Page> {
    // Auto-lock publish pages to prevent concurrent access
    await acquirePageLock(name);
  ...
  return {
    ...
    async disconnect(): Promise<void> {
      // Release any publish page locks held by this process
      releaseAllLocks();
