...
export async function serve(options: ServeOptions = {}): Promise<DevBrowserServer> {
  const port = options.port ?? 18273;
  const headless = options.headless ?? false;
  const cdpPort = options.cdpPort ?? 18274;
  ...
  // POST /pages/:name/set-file - set file on input element (bypasses CDP 50MB limit)
  // This runs directly on the server's page object, so no file transfer over CDP
  app.post("/pages/:name/set-file", async (req: Request<{ name: string }>, res: Response) => {
    const name = decodeURIComponent(req.params.name);
    const { filePaths, selector, frameUrl } = req.body as {
    ...
    for (const p of paths) {
      if (!existsSync(p)) {
        res.status(400).json({ error: `File not found: ${p}` });
        return;
      }
    }
    ...
    try {
      ...
      // If frameUrl is specified, find the matching frame
      let target: Page | ReturnType<Page["frame"]> = page;
      if (frameUrl) {
        const frame = page.frames().find(f => f.url().includes(frameUrl));
        if (!frame) {
          res.status(400).json({
            error: `Frame not found matching: ${frameUrl}`,
            availableFrames: page.frames().map(f => f.url()),
          });
          return;
        }
        target = frame;
      ...
      // Try locator-based approach first
      const locator = target.locator(sel);
      const count = await locator.count();

      if (count > 0) {
        await locator.first().setInputFiles(paths);
        res.json({ success: true, method: "locator", selector: sel, fileCount: paths.length });
        return;
      }

      // Fallback: find input via evaluate and use filechooser
      const hasInput = await target.evaluate((s: string) => {
        return !!document.querySelector(s);
      }, sel);

      if (hasInput) {
        // Element exists but locator can't find it (e.g., wujie shadow DOM)
        // Use filechooser event triggered by clicking the upload area
        const [fileChooser] = await Promise.all([
          page.waitForEvent('filechooser', { timeout: 10000 }),
          target.evaluate((s: string) => {
            const el = document.querySelector(s) as HTMLElement;
            if (el) el.click();
          }, sel),
        ]);
        await fileChooser.setFiles(paths);
        res.json({ success: true, method: "filechooser", selector: sel, fileCount: paths.length });
        return;
      }

      res.status(400).json({ error: `No element found matching selector: ${sel}` });
    } catch (err) {
