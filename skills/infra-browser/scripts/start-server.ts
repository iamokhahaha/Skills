...
try {
  const res = await fetch("http://localhost:18273", {
    signal: AbortSignal.timeout(1000),
  });
  if (res.ok) {
    console.log("Server already running on port 18273");
    process.exit(0);
  }
} catch {
...
// This handles the case where Node crashed but Chrome is still running on 18274
try {
  const pid = execSync("lsof -ti:18274", { encoding: "utf-8" }).trim();
  if (pid) {
    console.log(`Cleaning up stale Chrome process on CDP port 18274 (PID: ${pid})`);
    execSync(`kill -9 ${pid}`);
  }
} catch {
