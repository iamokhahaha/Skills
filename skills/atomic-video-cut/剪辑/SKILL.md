...
**必须使用 filter_complex + trim/atrim**，不要用 concat demuxer：

```bash
# ❌ 错误方法（会导致音画不同步）
ffmpeg -f concat -i segments.txt -c copy output.mp4

# ✅ 正确方法（trim+atrim 同时处理视频和音频）
...
间隔 <0.5秒 的片段应合并，减少处理量：

```python
merged = []
for start, end in segments:
    if merged and start - merged[-1][1] < 0.5:
        merged[-1] = (merged[-1][0], end)  # 合并
    else:
...
### 2026-01-24
- **音画不同步**：使用 concat demuxer 导出后音频和视频不同步
  - 原因：concat demuxer 分开处理视频和音频，时间戳可能有偏差
  - 正确：使用 filter_complex 的 trim+atrim 同时处理视频和音频
  - 命令：必须加 `-async 1` 确保音频同步
- **时长元数据错误**：导出后显示原始时长而非实际时长
  - 解决：使用 filter_complex 方法自动修正
- **优化**：合并间隔 <0.5秒 的相邻片段，减少 FFmpeg 处理负担
